...
F1TENTH ROS code.
"""
MAP_NAME = "pouliot_insane_map"
#### BOUTE DE JT
import gen_params
##liste de liste: prep,reg,interpolation
stepsizes = [[0.5,1.5,1.0],[0.25,0.75,0.5]]
## liste de vitesses maximales
vmaxes = [8.0,9.0,10.0]
##largeur (veh+safety)
largeurs = [0.3]
## mus
...
curvs_min = [3.0,2.0]
## opt_type
opt_types = ['mintime','mincurv']
##
params_ini = gen_params.Params_Ini(stepsizes,vmaxes,largeurs,mus,curvs_min,opt_types)
fichiers_params = gen_params.gen_ini(params_ini)
for fichier,mon_opt,mon_out in fichiers_params:
    print(fichier)
    try:
        # ----------------------------------------------------------------------------------------------------------------------
        # USER INPUT -----------------------------------------------------------------------------------------------------------
        ...
        file_paths = {"veh_params_file": f"{fichier}"}
        ...
        imp_opts = {"flip_imp_track": False,                # flip imported track to reverse direction
                    ...
                    "num_laps": 10}                          # number of laps to be driven (significant with powertrain-option),
        ...
        file_paths["traj_race_export"] = os.path.join(file_paths["module"], f"outputs/{MAP_NAME}", f"{mon_out}.csv")
        ...
        if plot_opts["racetraj_vel"]:
            ...
            #.plot(s_points, vx_profile_opt)
            #.plot(s_points, ax_profile_opt)
            #.plot(s_points, t_profile_cl[:-1])

            #.grid()
            #.xlabel("distance in m")
            #.legend(["vx in m/s", "ax in m/s2", "t in s"])
